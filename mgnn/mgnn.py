...
class MGNN(nn.Module):
    def __init__(self, args):
        ...
        self.query_emb = self.get_emb(args.query_vocab_size, args.query_emb_size, args.pad_id, 
                                      args.query_emb, args.update_query_emb)
        self.syntax_emb = self.get_emb(args.syntax_vocab_size, args.syntax_emb_size, args.pad_id, 
                                       args.syntax_emb, args.update_syntax_emb)
        self.hier_emb = self.get_emb(args.hier_vocab_size, args.hier_emb_size, args.pad_id, 
                                     args.hier_emb, args.update_hier_emb)
        self.rel_emb = self.get_emb(args.rel_vocab_size, args.rel_emb_size, args.pad_id, 
                                     args.rel_emb, args.update_rel_emb)
        self.ans_emb = self.get_emb(args.ans_vocab_size, args.ans_emb_size, args.pad_id, 
                                     args.ans_emb, args.update_ans_emb)
        ...
        self.fc = nn.Linear(1*args.filter_num*3 + args.rel_emb_size + args.ans_emb_size, 1, True)


    def get_emb(self, vocab_size, emb_size, pad_id, pre_emb, update):
        emb = nn.Embedding(vocab_size, emb_size, padding_idx=pad_id)
        if pre_emb:
            emb.weight = nn.Parameter(torch.from_numpy(pre_emb))
    ...
    def lstm_process(self, inputs, input_lengths, model, input_source):
        if input_source == 'query':
            ...
            input_lengths = input_lengths.data.numpy()
    ...
    def mask(self, inputs, input_lengths):
        ...
        seq_len = inputs.size(2); batch_size = inputs.size(0)
    ...
    def forward(self, query, query_length, syntax, syntax_length, hier, hier_length, rel, rel_length, ans, ans_length):
        
        query = self.query_emb(query)
        syntax = self.syntax_emb(syntax)
        hier = self.hier_emb(hier)
        rel = self.rel_emb(rel)
        ans = self.ans_emb(ans)
        ...
        ans = self.mask(ans, ans_length)
        
        query = self.query_cnn(query)
        syntax = self.syntax_cnn(syntax)
        hier = self.hier_cnn(hier)
        ans = torch.sum(ans, 1) / ans_length.unsqueeze(1).type(torch.FloatTensor)
        rel = rel.squeeze(1)

        ques = torch.cat([query, syntax], 1)
        pred = torch.cat([hier, rel, ans], 1)
